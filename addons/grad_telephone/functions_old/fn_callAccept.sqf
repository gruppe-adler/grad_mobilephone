...
if (isNull _objectCaller) exitWith { if (DEBUG_MODE) then {diag_log format ["_objectCalled didnt find a caller %1", _objectCalled];}; };
...
[_myFrequency, _myCode] remoteExec ["GRAD_fnc_setCallersPhoneFrequency", _objectCaller, false];